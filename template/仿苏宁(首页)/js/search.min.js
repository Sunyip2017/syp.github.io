...
SFE.search = (function($) {
    ...
    var snbookHotWord = function() {
        $.ajax({url: sn.searchDomain + "web/allHotWord.jsonp", cache: false, dataType: "jsonp", jsonp: "callback", jsonpCallback: "AllHWord", success: function(data) {
                var _str = "";
    ...
    var getSearchKeyword = function() {
        ...
        if (channelName == "snbook") {
            ...
            if (typeof searchKeywordsHidden != "undefined" && searchKeywordsHidden != "") {
                ...
                $.ajax({url: sn.searchDomain + "web/hotkeywords.jsonp", cache: false, dataType: "jsonp", jsonp: "callback", jsonpCallback: "HWord", success: function(data) {
        ...
        $.ajax({url: daMain + getCategoryId() + "--showHotkeywords" + ".xjsonp", type: "get", dataType: "jsonp", jsonpCallback: "showHotkeywords", success: function(data) {
                try {
                    ...
                    if (typeof searchKeywordsHidden != "undefined" && searchKeywordsHidden != "") {
                        ...
                        if ($("#searchKeywords").val() == "" && document.activeElement.id != "searchKeywords") {
                            ...
                            if (searchDefaultKeyword != "") {
                                ...
                                $("#searchKeywords").val("输入关键字搜索商品/品牌/店铺")
                            }
                            searchFlag = false
                        }
                    }
    ...
    var searchInputEvent = function() {
        ...
        obj.focus(function() {
            ...
            if (value == searchDefaultKeyword || value == "输入关键字搜索商品/品牌/店铺") {
            ...
            if (searchDefaultKeyword == "") {
                searchDefaultKeyword = "输入关键字搜索商品/品牌/店铺"
            }
            searchFlag = true;
            if (value == "") {
                searchFlag = false;
    ...
    var searchHotkeywords = function() {
        ...
        $.ajax({url: daMain + getCategoryId() + "-searchHotkeywords" + ".jsonp", type: "get", dataType: "jsonp", cache: true, jsonpCallback: "searchHotkeywords", success: function(data) {
                ...
                $(data).each(function() {
                    ...
                    dom += "<li>" + '<a id="public0_zzrs_' + namekey + "_" + index_num + "-" + dirId + '"  name="public0_zzrs_' + namekey + "_" + index_num + "-" + dirId + '" target="_blank" href="' + url + '" title="' + titlekey + '">' + tempkey + "</a>" + "</li>";
                    index_num++
                });
                searchHot.html(dom);
                var rec = $("#rec_results");
    ...
    var searchHisKeyword = function() {
        ...
        $.ajax({url: daMain + "his/" + userId + "---searchHis.jsonp", dataType: "jsonp", jsonpCallback: "searchHis", success: function(data) {
                if ($(data.words).size() <= 0) {
                    ...
                    $(data.words).each(function(key, value) {
                        if (typeof value.type != "undefined" && value.type === "his") {
                            ...
                            dom += '<li class="active"><span><a title="' + _title + '">' + _tempkey + '</a><i class="ng-iconfont">&#xe62d;</i></span></li>'
                        }
                    });
                    searchHis.html(dom);
                    $(".clear-history").click(function() {
                        ...
                        var userId = d("custno") || "";
                        var daMain = dsservice();
                        var url = daMain + "his/clean/" + userId + "-0.jsonp";
    ...
    var cancelbtnEvent = function() {
        ...
        $(tempkeyArray).each(function() {
            ...
            dom += '<li class="active"><span><a title="' + this + '">' + _tempkey + '</a><i class="ng-iconfont">&#xe62d;</i></span></li>'
        });
        searchHis.html(dom);
        $(".clear-history").click(function() {
            var e = e || window.event || arguments.callee.caller.arguments[0];
        ...
        var url = daMain + "his/add/" + userId + "-" + _str + "-0.jsonp";
        httpGifSendIndex(url)
    };
    var deleteHisKey = function(key) {
        var userId = d("custno") || "";
        var daMain = dsservice();
        var url = daMain + "his/del/" + userId + "-" + encodeURIComponent(key) + "-delsearchHis.jsonp";
        httpGifSendIndex(url)
    };
    ...
    var searchAutoComplete = function() {
        ...
        var autoComplateFun = function(keyword) {
            ...
            timer = setTimeout(function() {
                ...
                var daMain = dsservice();
                $.ajax({url: daMain + "his/" + userId + "-" + encodeURIComponent(resolveHisKey(keyword)) + "-" + getSearchCategoryId() + "-autoComplateCallback.jsonp", dataType: "jsonp", jsonpCallback: "autoComplateCallback", success: function(data) {
                        resultBox.show();
                        var resultDom = '<i class="close ng-iconfont">&#xe627;</i><ul>';
                        var i = 1;
                        var hisi = 1;
                        var n = 1;
                        var brandShop = data.brandShop;
                        if (brandShop && brandShop.shopName && brandShop.apsClickUrl && brandShop.shopLogo) {
                            resultDom += '<li class="g-ac-rec-store" assword=' + brandShop.shopName + ' apsClickUrl="' + brandShop.apsClickUrl + '"  name="brandshop" >' + '<a href="' + brandShop.apsClickUrl + '" target="_blank" class="store-link"><img src="' + brandShop.shopLogo + '" alt="" width="90" height="30"><em class="store-name">' + brandShop.shopName + '</em><i class="ng-iconfont arrow-r"></i></a><span style="display:none;"  class="keyparam">' + brandShop.shopName + "</span></li>"
                        }
                        $(data.words).each(function(key, value) {
                            if (typeof value.categoryName != "undefined") {
                                resultDom += "<li  assword=" + value.categoryName + ' name="category" indexnum="' + i++ + '"  ismanual= "' + value.isManual + '"  categoryid="' + value.categoryId + '" class="cateSearch">在<b>' + value.categoryName + '</b>分类 中搜索<span style="display:none;" class="keyparam">' + value.keyname + "</span></li>"
                            } else {
                                if (typeof value.type != "undefined" && value.type === "his") {
                                    var _tempkey = value.keyname.replace(/\</g, "&lt;").replace(/\>/g, "&gt;").replace(/\"/g, "&quot;");
                                    resultDom += "<li assword=" + _tempkey + '  name="history" indexnum="' + hisi++ + '"><span class="history">' + _tempkey + '</span><span style="display:none;"  class="keyparam">' + _tempkey + '</span><i class="tip">搜索历史</i><i class="ng-iconfont action">&#xe62d;</i></li>'
                                } else {
                                    if (n <= 8) {
                                        resultDom += "<li assword=" + value.keyname.replace(/<[^>]+>/g, "") + ' name="hotword" indexnum="' + i + '" ismanual= "' + value.isManual + '" ><span class="keyname">' + value.keyname + '</span><span style="display:none;"  class="keyparam">' + value.keyname + "</span></li>";
                                        i++;
                                        n++
                                    }
                                }
                        ...
                        keywordShow = keywordShow.replace(/\</g, "&lt;").replace(/\>/g, "&gt;");
                        keyword = keyword.replace(/\</g, "&lt;").replace(/\>/g, "&gt;");
                        resultDom += "<li assword=" + keywordShow + '   name="store" indexnum="1" class="g-ac-store"><i class="icon-store"></i>找“<em>' + keywordShow + '</em>”相关<b>店铺</b><span style="display:none;" class="keyparam">' + keyword + "</span></li>";
                        resultDom += "</ul>";
    ...
    function getCategoryId() {
        ...
        if (null != urlForSug && "" != urlForSug) {
            ...
            if (null != urlForSugTmp && urlForSugTmp[0].match("(^/strd$)|(^/sprd$)|(^/prd$)|(^/ProductDisplay$)|(^/cd$)|(^/pcd$)|(^/wine$)|(^/spbf$)")) {
                if (urlForSugTmp[0].match("(^/strd$)|(^/cd$)|(^/spbf$)")) {
                    categoryId = sn.categoryId
                } else {
                    if (urlForSugTmp[0].match("(^/wine$)")) {
                        categoryId = "0"
                    } else {
                        if (urlForSugTmp[0].match("(^/pcd$)")) {
                            categoryId = "0"
                        } else {
                            if (urlForSugTmp[0].match("(^/prd$)") || urlForSugTmp[0].match("(^/sprd$)")) {
                                categoryId = sn.categoryId
                            } else {
                                categoryId = sn.categoryId
                            }
                ...
                if (typeof (sn.pgFlag) != "undefined" && null != sn.pgFlag) {
                    if (sn.pgFlag == "10051_2") {
                        categoryId = sn.categoryId
                    } else {
                        if (sn.pgFlag == "10051_2.5") {
                            categoryId = "0"
                        } else {
                            if (sn.pgFlag == "10051_4") {
                                categoryId = sn.categoryId
                            }
                        }
                    }
                } else {
                    if (window.location.href.match("(.*?redbaby.*?)") != null || window.location.href.match("(.*?14655.*?)") != null || window.location.href.match("(.*?mysec.*?)") != null || window.location.href.match("(.*?mytop.*?)") != null || window.location.href.match("(.*?guide.*?)") != null) {
                        categoryId = "2"
                    } else {
                        if (window.location.href.match("(.*?binggo.*?)") != null || window.location.href.match("(.*?14656.*?)") != null || window.location.href.match("(.*?mztop.*?)") != null || window.location.href.match("(.*?mzsec.*?)") != null) {
                            ...
                            if (window.location.href.match("(.*?dianqicheng.*?)") != null) {
                                categoryId = "22005"
                            } else {
    ...
    var onSubmitSearch = function() {
        ...
        if (searchFlag) {
            ...
            if ($("#searchKeywords").val() === "输入关键字搜索商品/品牌/店铺") {
                return false
            }
        ...
        if (tmp == "") {
            ...
            try {
                ...
                if (nameType == "hotword") {
                    name += "-01-1_" + nameAndIndex + manual
                } else {
    ...
    return{onSubmitSearch: onSubmitSearch, searchEvent: searchEvent, cancelbtnEvent: cancelbtnEvent, clearBox: clearBox, searchIptBlur: searchIptBlur}
})(jQuery);